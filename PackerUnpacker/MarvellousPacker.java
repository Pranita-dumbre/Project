    ...
    public void PackingActivity()
    {
        try
        {
            ...
            if((fobj.exists()) && (fobj.isDirectory()))
            {
                ...
                System.out.println("Packed file gets succesfulliy created with name : "+PackName);
                ...
                for(i = 0; i < Arr.length; i++)
                {
                    ...
                    //Opne file from directory for reading
                    FileInputStream fiobj = new FileInputStream(Arr[i]);

                    //Write contents into pack file
                    while((iRet = fiobj.read(Buffer)) != -1)
                    {
                        foobj.write(Buffer,0,iRet);
                        System.out.println("File name Scaned : "+Arr[i].getName());
